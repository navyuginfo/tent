...
$.jgrid.extend({
	setColumns : function(p) {
		p = $.extend({
			top : 0,
			width: 200,
			left: this.jqGrid().width()- p.width -20,
			height: 'auto',
			dataheight: 'auto',
			modal: false,
			drag: true,
		...
		return this.each(function(){
			...
			if ( $("#"+IDs.themodal).html() != null ) {
				...
				bC  = p.showCancel ? "<a href='javascript:void(0)' id='eData' class='fm-button ui-state-default ui-corner-all'>"+p.bCancel+"</a>" : "";
				...
				if(!p.updateAfterCheck) {
					...
					$(":input","#"+dtbl).click(function(e){
						...
						if(cn){
							/* PR : modified to use the pre-hidden width */
